...
char train_file[MAX_STRING], output_file[MAX_STRING], output_file_vec[MAX_STRING], output_file_bin[MAX_STRING], pretrained_file[MAX_STRING];
...
void LearnVocabFromTrainFile() {
  ...
  long long a, i;
	...
	printf("Loading vocabulary from training file...\n");	
...
int InitNet() {
  long long a, b, wordIndex;
	long long binFileNumWords, binFileVecSize;  // from pt file
  ...
  if (*pretrained_file) { 
  	...
  	posix_memalign((void **)&pt_syn0, 128, binFileNumWords* layer1_size * sizeof(real));
  	...
  	for (i = 0; i < binFileNumWords; i++) {
    	...
    	wordIndex = SearchVocab(word);
			if (wordIndex >= 0) {
    		fread(&syn0[wordIndex * layer1_size], sizeof(real), layer1_size, f); // read layer1_size floats from this line following the space
    		fgetc(f);  // read the trailing eol
			}
			else {  // this word is not a part of the vocab that we read from the training file
    		...
    		fgetc(f);  // read the trailing eol
				...
				j = 0;
				while (*(word + j)) {
					pt_word_buff[MAX_STRING*pt_vocab_words + j] = *(word + j);
					j++;
				}
				pt_word_buff[MAX_STRING*pt_vocab_words + j] = 0;
				*/
		...
		printf("WARNING: Found %ld words from pre-trained file that don't exist in training file %s\n", pt_vocab_words, pretrained_file); 
  ...
  if (classes == 0) {
    ...
    for (a = 0; a < vocab_size; a++) {
      ...
      fprintf(fo2, "%s ", vocab[a].word);
			...
			fwrite(&syn0[a * layer1_size], sizeof(real), layer1_size, fo);
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-pt <file> ; Use pretrained model\n");
    printf("\nExamples:\n");
    printf("./word2vec -train data.txt -output vec.txt -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -cbow 1 -iter 3 -pt <vec1.bin>\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-pt", argc, argv)) > 0) strcpy(pretrained_file, argv[i + 1]); // load pre-trained wevecs
